...
class Product_Filter {
    ...
    private function get_wc_attributes() {
        ...
        global $wpdb;
        return $wpdb->get_results("SELECT * FROM {$wpdb->prefix}woocommerce_attribute_taxonomies");
    }
            ...
            foreach ($attributes as $attribute) {
                ...
                if (!empty($options)) {
                    foreach ($options as $option) {
                        if (!in_array($option, $custom_attributes[$attr_name])) {
        ...
        <div class="filter-sidebar">
            ...
            <div class="filter-group">
                ...
                <input type="text" id="filter-search" class="filter-select" 
                       placeholder="<?php _e('Search products...', 'hello-elementor-child'); ?>" 
            ...
            <div class="filter-group">
                ...
                <select id="filter-sort" class="filter-select">
                    ...
                    <option value="popularity"><?php _e('Popularity', 'hello-elementor-child'); ?></option>
            ...
            <?php if (!empty($wc_attributes)) : ?>
                ...
                ?>
                <div class="filter-group">
                    <label><?php echo esc_html(ucfirst($attribute->attribute_label)); ?></label>
                    <div class="filter-checkboxes">
                        <?php foreach ($terms as $term) : ?>
                        <div class="filter-checkbox-item">
                            <input type="checkbox" class="filter-attribute" 
                                   value="<?php echo esc_attr($taxonomy . ':' . $term->term_id); ?>"
                                   id="attr-<?php echo esc_attr($taxonomy . '-' . $term->term_id); ?>">
                            <label for="attr-<?php echo esc_attr($taxonomy . '-' . $term->term_id); ?>">
            ...
            <?php if (!empty($custom_attributes)) : ?>
                ...
                ?>
                <div class="filter-group">
                    <label><?php echo esc_html(ucfirst(str_replace(['-', '_'], ' ', $attr_name))); ?></label>
                    <div class="filter-checkboxes">
                        <?php foreach ($options as $option) : ?>
                        <div class="filter-checkbox-item">
                            <input type="checkbox" class="filter-custom-attribute" 
                                   value="<?php echo esc_attr($attr_name . ':' . $option); ?>"
                                   id="custom-attr-<?php echo esc_attr(sanitize_title($attr_name . '-' . $option)); ?>">
                            <label for="custom-attr-<?php echo esc_attr(sanitize_title($attr_name . '-' . $option)); ?>">
            ...
            <div class="filter-group">
                ...
                <div class="price-slider-container">
                    <div class="price-slider-wrapper">
                        ...
                        <input type="range" id="price-min-slider" min="0" max="<?php echo esc_attr($max_price); ?>" value="0" step="100">
                        <input type="range" id="price-max-slider" min="0" max="<?php echo esc_attr($max_price); ?>" value="<?php echo esc_attr($max_price); ?>" step="100">
                    </div>
                    <div class="price-values">
                        <span id="price-min-value">฿0</span>
                        <span id="price-max-value" data-max="<?php echo esc_attr($max_price); ?>">฿<?php echo number_format($max_price); ?></span>
                    </div>
                    <div class="price-inputs">
                        <input type="number" id="price-min-input" class="price-input" placeholder="Min" value="0" step="100">
                        <input type="number" id="price-max-input" class="price-input" placeholder="Max" value="<?php echo esc_attr($max_price); ?>" step="100">
                    </div>
                </div>
            </div>
            
            <button id="reset-filters" class="reset-btn"><?php _e('Reset Filters', 'hello-elementor-child'); ?></button>
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    public function render_shortcode($atts) {
        ...
        $url_props = isset($_GET['e_search_props']) ? sanitize_text_field($_GET['e_search_props']) : '';
        ...
        <div class="custom-product-filter-wrapper" 
            ...
            <div class="products-area">
                ...
                <div class="products-grid" data-columns="<?php echo esc_attr($atts['columns']); ?>">
                    <?php 
                    $initial_args = $atts;
                    if (!empty($url_search)) {
    ...
    public function get_products_html($args = []) {
        require_once get_stylesheet_directory() . '/includes/product-filter-query.php';
