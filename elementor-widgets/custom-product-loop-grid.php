...
class Elementor_Custom_Product_Loop_Grid extends \Elementor\Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'options' => [
                    ...
                    'top_rated' => __('Top Rated Products', 'hello-elementor-child'),
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .custom-product-loop-grid' => 'grid-template-columns: repeat({{VALUE}}, 1fr);',
                ],
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .custom-product-loop-grid' => 'column-gap: {{SIZE}}{{UNIT}};',
                ],
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .custom-product-loop-grid' => 'row-gap: {{SIZE}}{{UNIT}};',
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'default' => 'no',
                'description' => __('Use an Elementor template for product cards', 'hello-elementor-child'),
            ]
        );

        // Get all Elementor templates
        $templates = \Elementor\Plugin::instance()->templates_manager->get_source('local')->get_items();
        $template_options = ['' => __('Select Template', 'hello-elementor-child')];
        ...
        $this->add_control(
            ...
            [
                ...
                'description' => __('Create a product card template in Templates > Add New', 'hello-elementor-child'),
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'raw' => __('<strong>How to use:</strong><br>1. Go to Templates > Saved Templates > Add New<br>2. Create your product card design<br>3. Use Dynamic Tags to show product data<br>4. Select that template above', 'hello-elementor-child'),
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .custom-product-loop-grid' => 'background-color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'em', '%'],
                'selectors' => [
                    '{{WRAPPER}} .custom-product-loop-grid' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
    ...
    protected function render() {
        ...
        if (!$products_query->have_posts()) {
            echo '<div class="no-products-found">' . __('No products found.', 'hello-elementor-child') . '</div>';
            return;
        }
        ...
        $grid_class .= ' elementor-grid-tablet-' . ($settings['columns_tablet'] ?? '2');
        $grid_class .= ' elementor-grid-mobile-' . ($settings['columns_mobile'] ?? '1');

        ?>
        <div class="custom-product-loop-wrapper" id="product-loop-<?php echo esc_attr($widget_id); ?>">
            <div class="<?php echo esc_attr($grid_class); ?>" 
                ...
                while ($products_query->have_posts()) {
                    ...
                    $product_data = $this->get_product_data_attributes($product);

                    ?>
                    <article class="e-loop-item product-loop-item product-id-<?php echo esc_attr($product->get_id()); ?>"
                             <?php echo $this->render_data_attributes($product_data); ?>>
                        
                        <?php
                        if ($settings['use_custom_template'] === 'yes' && !empty($settings['template_id'])) {
                            // Render Elementor template
                            $this->render_elementor_template($settings['template_id']);
                        } else {
        ...
        <style>
            #product-loop-<?php echo esc_attr($widget_id); ?> .custom-product-loop-grid {
                display: grid;
                width: 100%;
            }
    ...
    private function build_query_args($settings) {
        ...
        return apply_filters('custom_product_loop_query_args', $args, $settings);
    }
    ...
    private function get_product_data_attributes($product) {
        ...
        if ($categories && !is_wp_error($categories)) {
            $data['categories'] = implode(',', wp_list_pluck($categories, 'term_id'));
        }
