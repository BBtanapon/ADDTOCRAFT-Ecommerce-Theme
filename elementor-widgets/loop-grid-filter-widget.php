...
class Elementor_Loop_Grid_Filter_Widget extends \Elementor\Widget_Base
{
	...
	protected function register_controls()
	{
		...
		$this->add_control("target_loop_grid_id", [
			...
			"description" => __(
				"Enter the CSS ID of the Loop Grid widget (Advanced > CSS ID)",
				"hello-elementor-child",
			),
		]);

		$this->add_control("filter_method", [
			"label" => __("Filter Method", "hello-elementor-child"),
			"type" => \Elementor\Controls_Manager::SELECT,
			"default" => "client_side",
			"options" => [
				"client_side" => __(
					"Client-Side (Fast, No Page Reload)",
					"hello-elementor-child",
				),
				"ajax" => __(
					"AJAX (Server-Side, More Accurate)",
					"hello-elementor-child",
				),
			],
			"description" => __(
				"Client-side is faster but works on visible items only",
				"hello-elementor-child",
			),
		]);
		...
		$this->add_control("mobile_toggle", [
	...
	protected function render()
	{
        ...
        <div class="loop-grid-filter-widget"
            ...
            data-target="<?php echo esc_attr(
            	$settings["target_loop_grid_id"],
            ); ?>"
            data-filter-method="<?php echo esc_attr(
            ...
            <div class="loop-filter-sidebar">
                ...
                <?php if ($settings["show_search"] === "yes"): ?>
                    <div class="filter-group">
                        ...
                        <input type="text" class="loop-filter-search filter-select" placeholder="<?php _e(
                        	"Search...",
                        	"hello-elementor-child",
                        ); ?>">
                    </div>
                        ...
                        <div class="filter-checkboxes">
                            <?php foreach ($categories as $category): ?>
                                <div class="filter-checkbox-item">
                                    <input type="checkbox" class="loop-filter-category" value="<?php echo esc_attr(
                                    	$category->term_id,
                                    ); ?>" id="cat-<?php echo esc_attr(
	$widget_id . "-" . $category->term_id,
); ?>">
                                    <label for="cat-<?php echo esc_attr(
                                    	$widget_id . "-" . $category->term_id,
                                    ); ?>"><?php echo esc_html(
	$category->name,
); ?></label>
                    ...
                    <div class="filter-group">
                        ...
                        <div class="filter-checkboxes">
                            <?php foreach ($tags as $tag): ?>
                                <div class="filter-checkbox-item">
                                    <input type="checkbox" class="loop-filter-tag" value="<?php echo esc_attr(
                                    	$tag->term_id,
                                    ); ?>" id="tag-<?php echo esc_attr(
	$widget_id . "-" . $tag->term_id,
); ?>">
                                    <label for="tag-<?php echo esc_attr(
                                    	$widget_id . "-" . $tag->term_id,
                                    ); ?>"><?php echo esc_html(
	$tag->name,
); ?></label>
                ...
                <?php if (
                	$settings["show_price_filter"] === "yes" &&
                    ...
                    <div class="filter-group">
                        ...
                        <div class="price-slider-container">
                            <div class="price-slider-wrapper">
                                ...
                                <input type="range" class="loop-price-min-slider" min="0" max="<?php echo esc_attr(
                                	$max_price,
                                ); ?>" value="0" step="100">
                                <input type="range" class="loop-price-max-slider" min="0" max="<?php echo esc_attr(
                                	$max_price,
                                ); ?>" value="<?php echo esc_attr(
	$max_price,
); ?>" step="100">
                            ...
                            <div class="price-values">
                                ...
                                <span class="price-max-value" data-max="<?php echo esc_attr(
                                	$max_price,
	...
	$max_price,
                ...
                <?php if ($settings["show_reset_button"] === "yes"): ?>
                    <button class="reset-btn loop-filter-reset"><?php _e(
                    	"Reset Filters",
                    	"hello-elementor-child",
                    ); ?></button>
                <?php endif; ?>
