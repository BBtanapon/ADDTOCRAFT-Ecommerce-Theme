...
class Elementor_Product_Image_Hover_Widget extends \Elementor\Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'show_indicators',
            [
                'label' => __('Show Pagination Indicators', 'hello-elementor-child'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'label_on' => __('Show', 'hello-elementor-child'),
                'label_off' => __('Hide', 'hello-elementor-child'),
                'return_value' => 'yes',
                'default' => 'yes',
                'description' => __('Display dots to show which image is currently visible', 'hello-elementor-child'),
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'options' => [
                    'woocommerce_thumbnail' => __('Thumbnail', 'hello-elementor-child'),
                    'woocommerce_single' => __('Single Product', 'hello-elementor-child'),
        ...
        $this->add_control(
            ...
            [
                ...
                'return_value' => 'yes',
                'default' => 'no',
                'description' => __('Make the entire gallery clickable', 'hello-elementor-child'),
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', '%'],
                'selectors' => [
                    '{{WRAPPER}} .product-hover-gallery' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                    '{{WRAPPER}} .product-hover-gallery img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => '#cccccc',
                'selectors' => [
                    '{{WRAPPER}} .gallery-indicator' => 'background-color: {{VALUE}};',
                ],
                'condition' => [
                    'show_indicators' => 'yes',
                ],
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => '#1e1e1e',
                'selectors' => [
                    '{{WRAPPER}} .gallery-indicator.active' => 'background-color: {{VALUE}};',
                ],
                'condition' => [
                    'show_indicators' => 'yes',
                ],
    ...
    protected function render() {
        ...
        if (!$product) {
            echo '<p>' . __('Please use this widget on a product page or within a product loop.', 'hello-elementor-child') . '</p>';
            return;
        }
        ...
        if (empty($all_images)) {
            echo '<p>' . __('No product images found.', 'hello-elementor-child') . '</p>';
            return;
        }
        ...
        if ($enable_link) {
            ...
            } elseif ($settings['link_type'] === 'custom' && !empty($settings['custom_link']['url'])) {
                $link_url = $settings['custom_link']['url'];
                $link_target = $settings['custom_link']['is_external'] ? '_blank' : '_self';
                $link_nofollow = $settings['custom_link']['nofollow'] ? 'nofollow' : '';
            }
        }
        
        ?>
        <div class="product-hover-gallery-wrapper" id="<?php echo esc_attr($widget_id); ?>">
            ...
            <div class="product-hover-gallery" data-effect="<?php echo esc_attr($hover_effect); ?>" data-auto-cycle="<?php echo $auto_cycle ? 'true' : 'false'; ?>" data-cycle-speed="<?php echo esc_attr($cycle_speed); ?>">
            ...
            <?php if ($show_indicators && count($all_images) > 1) : ?>
                <div class="gallery-indicators">
                    <?php foreach ($all_images as $index => $image_id) : ?>
                        <span class="gallery-indicator <?php echo $index === 0 ? 'active' : ''; ?>" data-index="<?php echo esc_attr($index); ?>"></span>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
        ...
        <style>
            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery-wrapper {
                position: relative;
                overflow: hidden;
            }
            ...
            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery img {
                ...
                object-fit: cover;
                opacity: 0;
                transition: opacity <?php echo esc_attr($transition_speed); ?>ms ease, transform <?php echo esc_attr($transition_speed); ?>ms ease;
            }

            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery img.active {
                opacity: 1;
            }

            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery[data-effect="zoom"] img.active {
                transform: scale(1);
            }

            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery[data-effect="zoom"]:hover img.active,
            #<?php echo esc_attr($widget_id); ?> .product-gallery-link:hover .product-hover-gallery[data-effect="zoom"] img.active {
                transform: scale(1.05);
            }

            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery[data-effect="slide"] img {
                transform: translateX(100%);
            }

            #<?php echo esc_attr($widget_id); ?> .product-hover-gallery[data-effect="slide"] img.active {
                transform: translateX(0);
            }

            #<?php echo esc_attr($widget_id); ?> .gallery-indicators {
                position: absolute;
                <?php echo $settings['indicators_position'] === 'top' ? 'top' : 'bottom'; ?>: 15px;
                left: 50%;
            ...
            @media (max-width: 768px) {
                ...
                #<?php echo esc_attr($widget_id); ?> .gallery-indicators {
                    ...
                    margin-top: 10px;
                    <?php echo $settings['indicators_position'] === 'top' ? 'top' : 'bottom'; ?>: auto;
                }
            }
        </style>
