...
function get_all_product_attributes($product)
{
    ...
    if ($product->is_type('variable')) {
        ...
        if (!empty($variation_prices['regular_price'])) {
            $attributes_data['regular_price'] = min($variation_prices['regular_price']);
        }
        if (!empty($variation_prices['sale_price']) && $product->is_on_sale()) {
            $attributes_data['sale_price'] = min($variation_prices['sale_price']);
        }
    } else {
        $attributes_data['regular_price'] = (float) $product->get_regular_price();
    ...
    foreach ($product_attributes as $attribute) {
        ...
        if (!empty($terms) && !is_wp_error($terms)) {
            $attributes_data['attributes'][$taxonomy] = wp_list_pluck($terms, 'slug');
        }
    }
